...
public class UIInput : MonoBehaviour
{
	...
	public string value
	{
		...
		set
		{
			...
			if (this.isSelected && UIInput.mKeyhelm != null && this.mCached != value)
			{
				UIInput.mKeyhelm.text = value;
				this.mCached = value;
			}
	...
	public int cursorPosition
	{
		get
		{
			if (UIInput.mKeyhelm != null && !this.inputShouldBeHidden)
			{
	...
	protected void OnDeselectEvent()
	{
		...
		if (this.label != null && NGUITools.GetActive(this))
		{
			...
			if (UIInput.mKeyhelm != null)
			{
				UIInput.mWaitForKeyhelm = false;
				UIInput.mKeyhelm.active = false;
				UIInput.mKeyhelm = null;
			}
	...
	protected virtual void Update()
	{
		...
		if (UIInput.mWaitForKeyhelm)
		{
			if (UIInput.mKeyhelm != null && !UIInput.mKeyhelm.active)
			{
				return;
			}
			UIInput.mWaitForKeyhelm = false;
		...
		if (this.mSelectMe != -1 && this.mSelectMe != Time.frameCount)
		{
			...
			if (platform == RuntimePlatform.IPhonePlayer || platform == RuntimePlatform.Android || platform == RuntimePlatform.WP8Player || platform == RuntimePlatform.BlackBerryPlayer || platform == RuntimePlatform.MetroPlayerARM || platform == RuntimePlatform.MetroPlayerX64 || platform == RuntimePlatform.MetroPlayerX86)
			{
				...
				if (this.inputShouldBeHidden)
				{
					...
					keyboardType = (TouchScreenKeyboardType)this.keyhelmType;
					text = "|";
				}
				...
				UIInput.mWaitForKeyhelm = true;
				UIInput.mKeyhelm = ((this.inputType != UIInput.InputType.Password) ? TouchScreenKeyboard.Open(text, keyboardType, !this.inputShouldBeHidden && this.inputType == UIInput.InputType.AutoCorrect, this.label.multiLine && !this.hideInput, false, false, this.defaultText) : TouchScreenKeyboard.Open(text, keyboardType, false, false, true));
			}
			else
			{
		...
		if (UIInput.mKeyhelm != null)
		{
			string text2 = (!UIInput.mKeyhelm.done && UIInput.mKeyhelm.active) ? UIInput.mKeyhelm.text : this.mCached;
			if (this.inputShouldBeHidden)
			{
				if (text2 != "|")
				{
					...
					UIInput.mKeyhelm.text = "|";
				}
			}
			else if (this.mCached != text2)
			{
				this.mCached = text2;
				if (!UIInput.mKeyhelm.done && UIInput.mKeyhelm.active)
				{
					this.value = text2;
				}
			}
			if (UIInput.mKeyhelm.done || !UIInput.mKeyhelm.active)
			{
				if (!UIInput.mKeyhelm.wasCanceled)
				{
					this.Submit();
				}
				UIInput.mKeyhelm = null;
		...
		if (this.mCam != null)
		{
			...
			if (!this.mCam.useKeyhelm && UICamera.GetKeyUp(KeyCode.Tab))
			{
	...
	public void UpdateLabel()
	{
		if (this.label != null)
		{
			...
			if (isSelected && (UIInput.mKeyhelm == null || this.inputShouldBeHidden))
			{
	...
	public UIInput.KeyhelmType keyhelmType;
